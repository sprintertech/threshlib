...
func (round *presign3) Start() *tss.Error {
	...
	for j, Pj := range round.Parties().IDs() {
		common.Logger.Infow(fmt.Sprintf("Round 3 parties. Party: %s ID: %d", Pj.Id, j), zap.String("SessionID", string(round.temp.sessionIdU.Bytes())))
	}
	...
	for j, Pj := range round.Parties().IDs() {
		...
		go func(j int, Pj *tss.PartyID) {
			common.Logger.Infow(fmt.Sprintf("Calculating delta share alpha. Party: %s ID: %d", Pj.Id, j), zap.String("SessionID", string(round.temp.sessionIdU.Bytes())))
			defer wg.Done()
			...
			common.Logger.Infow(fmt.Sprintf("Setting delta share alpha. %s. Party: %s ID: %d", AlphaDelta, Pj.Id, j), zap.String("SessionID", string(round.temp.sessionIdU.Bytes())))
	...
	for j := range round.Parties().IDs() {
		...
		common.Logger.Infow(
			fmt.Sprintf(
				...
				j,
			), zap.String("SessionID", string(round.temp.sessionIdU.Bytes())))
